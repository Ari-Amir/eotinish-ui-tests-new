...
class ExecuteAppealsGroupTwoWithHearing (private val assert: Boolean = true) {
    ...
    fun executeAppealsGroupTwoWithHearing() {
        ...
        for (i in regNumbersOfCreatedAppeals.indices) {
            ...
            //Проверяем корректно ли отображаются все типы решений и все типы причин
            if (assert) {
      ...
      */
        if (assert) {
            ...
            println("Test 3.3.1.2 passed (${typesOfCreatedAppeals[0]}, ЖТ-${regNumbersOfCreatedAppeals[0]})")
