...
//ПРОДОЛЖЕНИЕ РАССМОТРЕНИЯ ОБРАЩЕНИЯ ИСПОЛНИТЕЛЕМ ПРИ ПОСТУПЛЕНИИ ОТЗЫВА ОТ РЕГИСТРАТОРА
...
class ContinueAppealExecutionAfterRevoke (
    ...
    fun continueAppealExecutionAfterRevokeTest() {
        ...
        //Нажимаем кнопку "Продолжить рассмотрение", указываем причину отказа отзыва обращения и нажимаем кнопку "Принять решение"
        ...
        /*
        ПРОВЕРКА:
        В статусах обращения должен отображаться статус "Исполнено"
        */
        if (assert) {
            assertEquals(org1Chief1.page.innerText(appealCardPage.statusAppealFinished), statusAppealFinished)
            println("Test 3.10.3  passed ($appealType ЖТ-$APPEAL_NUMBER)")
        }
    }
}
